...
(75–90%) until ~500. Right: the root value estimate stays near −0.26
regardless of budget — MCTS with ±1 terminal outcomes estimates a win
*probability*, not a value magnitude. Generated by
`phase4_hybrid/lesson044_9_mcts/make_figures_49.py` (same game, fewer games per point than the
table's 40, so the pure-MCTS wobble is sampling noise — the guided curve
saturating immediately is the signal).*
