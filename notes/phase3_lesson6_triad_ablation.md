...
1. **Real divergence captured**: removing clipping + raising lr to 3e-3 +
   removing replay/target blows `max|Q|` to 3.2e8 (vs ~440 for every healthy
   variant) — the Q function literally runs away. This is the deadly triad
   photographed in the wild, not asserted.
