...
This repo backs a guided, compact course (33 lessons): each concept first
...
| 4.6 | Risk-sensitive & distributional RL (CVaR policy selection) | `lesson4_6_risk_sensitive.py` |
...
Figures: `assets/phase{1..4}/` — every figure is regenerated by a seeded
`make_figures*.py` script next to the lesson that owns it.
...
- **Tooling:** Python 3.11, NumPy, SciPy (`scipy.optimize.milp` / `linprog`),
  OR-Tools 9.15 (pywraplp, CP-SAT, Routing, graph flows), Pyomo, Gymnasium,
