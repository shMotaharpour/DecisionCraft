...
`phase1_milp/lesson003_2b_order_symmetry/make_figures_12b.py`.*
...
1. Add a third identical warehouse and re-run: orbit size goes C(4,2)=6
   → C(5,2)=10; measure solver time with and without the ordering
   constraint and report the widening gap.
2. Break symmetry with `x1 + x2 + x3 <= 1`-style *clique* cuts instead of
   ordering; compare node counts — which symmbreak is stronger here?
3. Add a tie in costs (two warehouses with identical parameters) and
   count the alternative optima with and without symmetry breaking.
4. Lesson 1.6c's GA ignores symmetry entirely; run it on the same
   instance and discuss why metaheuristics are immune to this disease.
