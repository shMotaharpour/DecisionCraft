...
Right: the price of that fixed budget — the optimality gap, ≤ 0.24 %
everywhere and shrinking as n grows (more items = more local moves to
choose from). Generated by `phase1_milp/lesson013_6c_genetic_algorithm/` companion generator (same seed and (same seed and
budget as the evidence run).*
