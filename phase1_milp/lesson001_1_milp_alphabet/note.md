...
feasible region is a polygon; sweeping iso-profit lines (dashed) shows the
optimum is a **vertex** — here (6,4) = 138, where the last binding
constraint is the exposure cap. Right: profit along the two edges
converging at that vertex; a linear objective can never peak inside.
Verified by direct vertex enumeration. Generated by
`phase1_milp/lesson001_1_milp_alphabet/make_figures_11.py`.*
