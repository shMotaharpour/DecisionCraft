...
monotonically from 0.905 (stake 1) to 0.26 (stake 5). Right: expected time
to absorption peaks at mid-stake (8.77 steps at stake 3) — the two panels
answer two different questions and peak in opposite ways. Generated by
`phase2_mdp/lesson023_5_absorbing_hitting/make_figures_25.py` (same model and numbers as the lesson run).*
...
market is 4× faster than the reverse, a one-glance asymmetry the numbers
in the text encode. Right: Kemeny's law as bars — mean return time to i
is exactly 1/πᵢ (proven), so lesson 2.1's stationary distribution doubles
as a return-time table. Generated by `phase2_mdp/lesson023_5_absorbing_hitting/make_figures_25b.py`
(same chain, first-step linear systems verified vs Monte Carlo).*
