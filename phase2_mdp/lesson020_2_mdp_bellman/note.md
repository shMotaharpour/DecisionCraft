...
Right: what converged — the discovered policy is the business rule
(order up to S = 5 whenever stock is below 5, zero otherwise) and V\*
rises with inventory, flattening exactly where re-ordering stops. The
solver was never told about (s,S). Generated by
`phase2_mdp/lesson020_2_mdp_bellman/make_figures_22.py`.*
