...
curve) descends faster and smoother; MC carries full-return noise. Right:
the final estimates against exact V — and the gray band marking states 6–10
that the (s,S) policy never visits: no on-policy method can learn there.
Generated by `phase2_mdp/lesson025_7_mc_td_cmdp/make_figures_27.py` (same model, policy, seed).*
